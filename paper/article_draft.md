...
**Results:** Quality filtering retained 187/616 patients (30.4%). On the filtered cohort, SegResNet significantly outperformed U-Net on all four OARs (Bonferroni-corrected paired Wilcoxon, all p < 0.001, all Wilcoxon's r between 0.52 and 0.87 — Cohen "large"): median DSC improved by +0.051 (left lung), +0.037 (right lung), +0.084 (heart), and +0.257 (esophagus, U-Net 0.41 → SegResNet 0.67). HD95 was reduced approximately threefold for all OARs (e.g., right lung 7.0 → 2.4 mm; esophagus 9.0 → 3.7 mm). Mean patient-level DSC across the four OARs reached 0.813 ± 0.109 for SegResNet versus 0.690 ± 0.157 for U-Net (r = 0.87, p < 0.001). [PLACEHOLDER: XAI sanity checks confirmed model dependence (SSIM < 0.X after cascading randomization).]
...
To quantify the impact of the quality filtering pipeline, all training and evaluation protocols were applied independently on (a) the unfiltered cohort (616 patients) and (b) the filtered cohort (187 patients), for both architectures. Paired comparisons between architectures and between dataset conditions were performed using the Wilcoxon signed-rank test on per-patient metric values. Bonferroni correction was applied to control the family-wise error rate across the four OARs tested per metric. Beyond the p-value, we report Wilcoxon's r effect size, defined as r = |Z| / √N where Z is the standardized test statistic and N the number of non-tied pairs, with the conventional thresholds r ≥ 0.10 (small), r ≥ 0.30 (medium) and r ≥ 0.50 (large) [Cohen 1988]. Effect-size reporting was added to complement the p-value because, at our sample size (n ≈ 187), even small differences would yield highly significant p-values; the effect size provides the magnitude of the effect, which is the more clinically relevant quantity.
...
For DSC, the median scores for SegResNet versus U-Net were 0.933 vs 0.881 for the left lung, 0.941 vs 0.904 for the right lung, 0.885 vs 0.801 for the heart, and 0.666 vs 0.409 for the esophagus. The four pairwise comparisons were all highly significant after Bonferroni correction (paired Wilcoxon signed-rank, p < 0.001 in every case), with Wilcoxon's r effect sizes between 0.78 and 0.85 — all comfortably within Cohen's "large" range. The size of the gain scaled with the difficulty of the organ: about +0.04 DSC for the lungs, +0.08 for the heart, and +0.26 for the esophagus.
